...
A big change with migration to versions of SDL 4.3 and later is the change from a delegate-based to a notification-based infrastructure. Basically, this means that all delegate callbacks relating to Responses and Notifications within `SDLProxyListener.h` will now be notifications, with their names listed in `SDLNotificationConstants.h`.

Also, we have added the ability to have completion handlers for when a request is sent and it's response comes back (or the ability to register for a notification when the response comes in). 

Because of this, any delegate function will become non-functional when migrating to `SDLManager` from `SDLProxy`, but changing these to use the new handlers is simple and will be described in the section **SDLRPCRequestFactory**.  
...
In versions following 4.3, the `SDLProxy` is no longer your main point of interaction with SDL. Instead, `SDLManager` was introduced to allow for the developers of SDL to more easily allow integration into their app, and to not worry about things such as registering, uploading, and showing a lock screen.

Our first step of removing the usage of `SDLProxy` is to add in an `SDLManager` instance variable. `SDLManager` is comprised of an `SDLConfiguration`, which contains settings relating to the application itself within `SDLLifecycleConfiguration`, and other features such as the lock screen in `SDLLockScreenConfiguration`.
...
Because of this, we can remove the following functions
- `hsdl_getNextCorrelationId`
...
Because we the way we configuration the app's properties via `SDLLifecycleConfiguration` now, we do not need to actually send an `SDLRegisterAppInterface` request. Because of this, we can remove the following functions, and their contents:
- `onProxyOpened`
